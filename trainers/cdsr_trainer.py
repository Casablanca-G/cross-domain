...
from models.IBMLCDSR import IBMLCDSR
from zujian.utils import record_csv, metric_report, metric_domain_report
...
class CDSRTrainer(SeqTrainer):
    ...
    def __init__(self, args, logger, writer, device, generator):
        ...
        # item_pop is available after super().__init__() completes
        item_numA = self.item_num_dict["0"]
        pop_A = torch.tensor(self.item_pop[:item_numA + 1], dtype=torch.float32)
        pop_B_raw = self.item_pop[item_numA + 1:]
        pop_B = torch.tensor(np.concatenate([[0], pop_B_raw]), dtype=torch.float32)
        self.model.register_popularity(pop_A, pop_B)
    ...
    def _create_model(self):
        ...
        elif self.args.model_name == "ibml_cdsr":
            self.model = IBMLCDSR(self.user_num, self.item_num_dict, self.device, self.args)
        else:
            raise ValueError
