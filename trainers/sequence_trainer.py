...
from zujian.utils import metric_report, metric_len_report, record_csv, metric_pop_report
from zujian.utils import metric_len_5group, metric_pop_5group
...
class SeqTrainer(Trainer):
    ...
    def _train_one_epoch(self, epoch):
        ...
        if hasattr(self.model, "get_monitor_info"):
            monitor = self.model.get_monitor_info()
            if monitor:
                for key, value in monitor.items():
                    self.writer.add_scalar('train/{}'.format(key), value, epoch)
                msg = ', '.join('{}={:.4f}'.format(k, v) for k, v in monitor.items())
                self.logger.info('Train monitor: ' + msg)
